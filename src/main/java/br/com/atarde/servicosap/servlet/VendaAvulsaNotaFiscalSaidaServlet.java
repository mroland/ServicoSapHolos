 ...
 */
@WebServlet("/VendaAvulsaNotaFiscalSaidaServlet")
public class VendaAvulsaNotaFiscalSaidaServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public VendaAvulsaNotaFiscalSaidaServlet() {
		super();
	...
	private void processar(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		if (TSUtil.isEmpty(empresaId)) {
			...
			if (TSUtil.isEmpty(empresa)) {
				...
				new VendaAvulsaNotaFiscalSaidaBusiness().inserirSAP(empresa);
