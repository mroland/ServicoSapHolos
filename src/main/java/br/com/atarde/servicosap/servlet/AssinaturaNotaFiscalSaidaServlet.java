 ...
 */
@WebServlet("/AssinaturaNotaFiscalSaidaServlet")
public class AssinaturaNotaFiscalSaidaServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public AssinaturaNotaFiscalSaidaServlet() {
		super();
	...
	private void processar(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		if (TSUtil.isEmpty(empresaId)) {
			...
			if (TSUtil.isEmpty(empresa)) {
				...
				new AssinaturaNotaFiscalSaidaBusiness().inserirSAP(empresa);
