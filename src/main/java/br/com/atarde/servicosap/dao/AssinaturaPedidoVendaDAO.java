 ...
 * @author mroland
 */
public class AssinaturaPedidoVendaDAO {

	public AssinaturaPedidoVendaDAO() {
	}
	...
	public AssinaturaNotaFiscalSaida inserirInterface(AssinaturaNotaFiscalSaida model) throws TSApplicationException {
		...
		for (NotaFiscalSaidaLinhaAB item : model.getLinhas()) {
			...
			new AssinaturaPedidoVendaLinhaDAO().inserirInterface(linha, broker);
		...
		if (!TSUtil.isEmpty(model.getParcelas())) {
			...
			for (ParcelaAB item : model.getParcelas()) {
				...
				new AssinaturaPedidoVendaParcelaDAO().inserirInterface(linha, broker);
	...
	public AssinaturaPedidoVenda obterIdExternoInterface(AssinaturaPedidoVenda model) {

		TSDataBaseBrokerIf broker = TSDataBaseBrokerFactory.getDataBaseBrokerIf();

		broker.setSQL("SELECT ID FROM ASSINATURAPEDIDOVENDA WHERE ID_EXTERNO = ? AND EMPRESA_ID = ?", model.getIdExterno(), model.getEmpresa().getId());

		return (AssinaturaPedidoVenda) broker.getObjectBean(AssinaturaPedidoVenda.class, "interfaceId");
	}
}
