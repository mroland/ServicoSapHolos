...
@Path(value = "/devolucaoNotaFiscalSaidaWS")
public class DevolucaoNotaFiscalSaidaWS {

	@POST
	@Consumes({ MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML })
	@Produces({ MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML })
	@Path(value = "/inserirLote")
	public Response inserirLote(@HeaderParam("Authorization") String token, DevolucaoNotaFiscalSaida model) {
		...
		if (token != null && token.startsWith("Bearer ")) {
			...
			if (TokenService.validateToken(token)) {
				...
				try {
					...
					NotaFiscalSaidaAB conta = new DevolucaoNotaFiscalSaidaBusiness().insertBatchEvent(model);
